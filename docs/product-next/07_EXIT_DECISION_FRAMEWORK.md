...
Convert exit **research** into **live deterministic triggers** that set recommendation `EXIT_APPROVED`, requiring **human confirmation** before any sell ([01](./01_RECOMMENDATION_ENGINE_PRD.md), [11](./11_HUMAN_IN_LOOP_EXECUTION_PRD.md)).
...
`S_exit_health` in [02](./02_CONVICTION_SCORING_PRD.md) drops to 15–20 when triggers fire, reinforcing EXIT_APPROVED.
