...
Manage **dynamic capital**, **regime-based position count**, **per-name allocation**, **risk limits**, and **cash** for the swing book. Feeds truthful `portfolio_context` into ARGS and position-aware recommendations ([01](./01_RECOMMENDATION_ENGINE_PRD.md)).

**Out of scope v1:** Live broker execution (integration points only — [11](./11_HUMAN_IN_LOOP_EXECUTION_PRD.md)).
