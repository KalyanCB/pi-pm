...
ARGS packets **must** include `recommendation` block ([01](./01_RECOMMENDATION_ENGINE_PRD.md)). Committees cite `recommendation:*` and `portfolio_context:*` evidence ([`rc.py`](../../app/args/plugins/rc.py), [`evidence_validator.py`](../../app/workspace_args/evidence_validator.py)).
...
| Measurement | Post-hoc in [16_RECOMMENDATION_PERFORMANCE_PRD.md](./16_RECOMMENDATION_PERFORMANCE_PRD.md) |
...
| No conviction influence | Cannot affect conviction or machine action ([02](./02_CONVICTION_SCORING_PRD.md)) |
