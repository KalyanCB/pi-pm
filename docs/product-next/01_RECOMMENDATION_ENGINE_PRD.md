...
It sits **after Validation** and **before ARGS** so committees review packets that already include a deterministic recommendation stub and conviction score ([04_RECOMMENDATION_LIFECYCLE.md](./04_RECOMMENDATION_LIFECYCLE.md)).
...
| Validation report | `app/validation/`, per `ranking_run_id` | Yes | Status `completed` or `insufficient_data` — latter caps conviction ([02](./02_CONVICTION_SCORING_PRD.md)) |
| Regime snapshot | `GET /observability/regime/current`, `regime_policy_decisions` | Yes | Gates max new BUYs ([05](./05_PORTFOLIO_ENGINE_PRD.md)) |
...
| `action` | enum | Above |
| `conviction_score` | 0–100 | [02_CONVICTION_SCORING_PRD.md](./02_CONVICTION_SCORING_PRD.md) |
| `conviction_band` | enum | e.g. HIGH / MEDIUM / LOW / BLOCKED |
| `reason_codes` | string[] | Machine-readable — canonical codes in [16_WHY_NOT_RECOMMENDED_FRAMEWORK.md](./16_WHY_NOT_RECOMMENDED_FRAMEWORK.md) |
| `lifecycle_state` | enum | [04](./04_RECOMMENDATION_LIFECYCLE.md) |
...
| R-ARGS-03 | CRO synthesis never sets `action` field — [08](./08_AI_INVESTMENT_COMMITTEE_PRD.md) |
...
| AC-RE-04 | `BUY` count ≤ regime-based max slots ([05](./05_PORTFOLIO_ENGINE_PRD.md)). |
...
See [04_RECOMMENDATION_LIFECYCLE.md](./04_RECOMMENDATION_LIFECYCLE.md) and [11_HUMAN_IN_LOOP_EXECUTION_PRD.md](./11_HUMAN_IN_LOOP_EXECUTION_PRD.md).
...
| GET | `/api/v1/recommendations/why-not/{symbol}` | Deterministic why-not-BUY ([16](./16_WHY_NOT_RECOMMENDED_FRAMEWORK.md)) |
...
| Data model + migrations | Spec [03](./03_RECOMMENDATION_DATA_MODEL.md) | M1 |
...
| ARGS “supportive” confusion | Separate labels; [08](./08_AI_INVESTMENT_COMMITTEE_PRD.md) |
...
- [VALIDATION_DESIGN.md](../AI/03_DESIGN/VALIDATION_DESIGN.md)
- [EXIT_RESEARCH_DESIGN.md](../AI/03_DESIGN/EXIT_RESEARCH_DESIGN.md)
- [outcome-attribution-report.md](../outcome-attribution-report.md)
- [16_WHY_NOT_RECOMMENDED_FRAMEWORK.md](./16_WHY_NOT_RECOMMENDED_FRAMEWORK.md)
