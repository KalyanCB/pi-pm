...
def test_upsert_daily_is_idempotent_within_session(db_session):
    ...
    repo.upsert_daily(row, strategy_name="breakout_v1", strategy_version="1.0.0", universe_code="PI_PM_CORE")
    row2 = DailyFactorIC(
        factor_name="volume_surge",
        ranking_run_id=run_id,
        as_of_date=date(2024, 6, 3),
    ...
    repo.upsert_daily(row2, strategy_name="breakout_v1", strategy_version="1.0.0", universe_code="PI_PM_CORE")
    db_session.commit()
    ...
    count = db_session.scalar(
        select(func.count()).select_from(FactorDailyMetric).where(
