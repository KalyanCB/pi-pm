...
def test_deterministic_replay():
    ...
    assert hash1 == hash2
    assert [(x.action, x.conviction_score) for x in r1] == [(x.action, x.conviction_score) for x in r2]
...
def _run_with_exit(rows, exit_signals=None, active_positions=None):
    cfg = EngineConfig(regime_posture="risk_on", rank_deterioration_threshold=40, max_holding_days=30)
    val = ValidationSummary(status="completed", ic_20d=0.06, top_decile_spread=0.02)
    results, _ = run(
        ranking_run_id=uuid.uuid4(),
        ranking_results=rows,
...
def test_alpha_decay_triggers_exit():
    ...
    results = _run_with_exit([row], active_positions={row.stock_id}, exit_signals={row.stock_id: sig})
    assert results[0].action == RecommendationAction.EXIT_APPROVED
