#!/usr/bin/env python3
"""Generate per-rank reliability report (research only, read-only DB)."""
...
from app.ranking_research.reports import build_rank_reliability_markdown
from app.ranking_research.service import RankReliabilityService

DEFAULT_OUTPUT = Path("docs/rank-reliability-report.md")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate rank reliability research report")
    ...
    try:
        ...
        report = RankReliabilityService().compute(config, observations, benchmarks)
        markdown = build_rank_reliability_markdown(report)
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(markdown, encoding="utf-8")
        print(f"Wrote {args.output} ({report.ranked_run_count} runs)")
    finally:
        db.close()
