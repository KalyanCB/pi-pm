...
def _svc(db=Depends(get_db), portfolio_id: PortfolioScope = ...) -> PortfolioService:
    return PortfolioService(db, portfolio_id=portfolio_id)


def _pt_svc(db=Depends(get_db), portfolio_id: PortfolioScope = ...) -> PaperTradeService:
...
def recompute(_owner: OwnerUser, svc: PortfolioService = Depends(_svc)) -> dict:
...
def get_reconciliation(svc: ReconciliationService = Depends(_recon_svc)) -> dict:
    ...
    if report is None:
        raise HTTPException(
            status_code=404, detail="No reconciliation report yet. Run POST /portfolio/reconcile."
        )
    return {
        ...
        out.append(
            {
                ...
                "unrealized_pnl_pct": float(r.unrealized_pnl_pct)
...
def get_dashboard(db=Depends(get_db)) -> Any:
    ...
    today_change = (
        float(nav_rows[-1].day_return_pct)
    ...
    try:
        ...
        symbol_rows = sorted(
            ...
            key=lambda b: b.contribution_pct or 0,
            reverse=True,
        )
