...
class StockSetupResearch(Base, UUIDPrimaryKeyMixin):
    ...
    qualifying_matches: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    setup_evidence_score: Mapped[float | None] = mapped_column(Numeric(6, 2), nullable=True)
...
class StockSetupResearchMetric(Base, UUIDPrimaryKeyMixin):
    ...
    standard_deviation_20d: Mapped[float | None] = mapped_column(Numeric(18, 8))
    max_return_20d: Mapped[float | None] = mapped_column(Numeric(18, 8))
    min_return_20d: Mapped[float | None] = mapped_column(Numeric(18, 8))
    confidence_interval_95_lower_20d: Mapped[float | None] = mapped_column(Numeric(18, 8))
    confidence_interval_95_upper_20d: Mapped[float | None] = mapped_column(Numeric(18, 8))
