...
from app.stock_setup_evidence.similarity import select_nearest_setups
...
class StockSetupResearchService:
    ...
    def run_for_candidate(
        ...
        *,
        ...
        reference = extract_reference_profile(result.score_components)
        if len(reference) < 4:
            return self._persist_insufficient(
                ranking_run, result, stock, reference, nearest_n, min_similarity,
        ...
        historical = build_stock_internal_normalized_profiles(
            snapshot, stock_series, bench_series or None, candidate_dates
        )
        matches = select_nearest_setups(
            reference, historical, nearest_n=nearest_n, min_similarity=min_similarity
        )
        ...
        regime_labels = {REGIME_LABEL_ALL_REGIMES}
        for outcome in outcomes:
            if outcome.regime_label:
                regime_labels.add(outcome.regime_label)

        metrics_payload: list[dict] = []
        for label in sorted(regime_labels):
            agg = aggregate_outcomes(outcomes, label)
            if agg.occurrence_count == 0:
                continue
            metrics_payload.append(
                {
                    ...
                    "avg_return_5d": agg.avg_return_5d,
                    "avg_return_20d": agg.avg_return_20d,
