...
_strategy = BreakoutV1Strategy()


def extract_reference_profile(score_components: dict[str, Any] | None) -> dict[str, float]:
    """Normalized factor vector from ranking score_components."""
    if not score_components:
        return {}
    profile: dict[str, float] = {}
    for name in SEE_FACTOR_NAMES:
...
def compute_raw_profile_at_date(
    ...
    raw = _strategy.compute_raw_factors(stock, price_series, benchmark_series, as_of_date)
    out: dict[str, float] = {}
    for name in SEE_FACTOR_NAMES:
        value = raw.get(name)
        if value is None:
...
def build_stock_internal_normalized_profiles(
    ...
    for d in candidate_dates:
        raw = compute_raw_profile_at_date(stock, price_series, benchmark_series, d)
        if len(raw) >= 4:
            raw_by_date[d] = raw
    ...
    factor_values: dict[str, list[float]] = {name: [] for name in SEE_FACTOR_NAMES}
